...
export default function ManageProjects() {
  ...
  const [formData, setFormData] = useState({ title: '', shortDescription: '', description: '', category: 'web-development', liveUrl: '', thumbnail: '' });
  ...
  const handleCancelEdit = () => {
    setFormData({ title: '', shortDescription: '', description: '', category: 'web-development', liveUrl: '', thumbnail: '' });
    setEditingId(null);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 800 }}>Manage Projects</h2>
        {!isFormVisible && (
      ...
      {isFormVisible && (
        <div style={{ 
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div className="grid grid-2" style={{ gap: '1rem' }}>
              <div><label>Live Link</label><input className="input-field" value={formData.liveUrl} onChange={e => setFormData({...formData, liveUrl: e.target.value})} /></div>
              <ImageUpload label="Project Thumbnail" folder="forallaxis/projects" value={formData.thumbnail} onChange={(url) => setFormData({...formData, thumbnail: url})} />
            </div>
            <div><label>Full Description</label><textarea className="input-field" rows="4" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} required /></div>
        ...
        <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'rgba(239, 68, 68, 0.05)', borderBottom: '1px solid var(--glass-border)' }}>
              ...
              <th style={{ padding: '1rem 1.5rem', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', fontSize: '0.75rem', letterSpacing: '1px' }}>Category</th>
              <th style={{ padding: '1rem 1.5rem', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', fontSize: '0.75rem', letterSpacing: '1px' }}>Featured</th>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item._id} style={{ borderBottom: '1px solid var(--glass-border)', transition: 'all 0.2s' }}>
                ...
                <td style={{ padding: '1rem 1.5rem' }}><span style={{ padding: '0.25rem 0.75rem', background: 'rgba(239, 68, 68, 0.1)', color: 'var(--accent-red)', borderRadius: '1rem', fontSize: '0.875rem' }}>{item.category}</span></td>
                <td style={{ padding: '1rem 1.5rem' }}>{item.featured ? '⭐ Yes' : 'No'}</td>
                <td style={{ padding: '1rem 1.5rem', textAlign: 'right' }}>
                  <button type="button" onClick={() => handleToggleFeatured(item._id)} className="btn btn-sm btn-outline" style={{ marginRight: '0.5rem', borderColor: 'var(--text-secondary)', color: 'var(--text-secondary)' }}>Toggle Featured</button>
