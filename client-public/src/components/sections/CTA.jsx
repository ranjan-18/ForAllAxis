...
const CTA = () => {
  return (
    <section style={{ position: 'relative', padding: '6rem 0', overflow: 'hidden', background: '#ffffff' }}>
      ...
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '100%', height: '100%', zIndex: 0, overflow: 'hidden' }}>
        <motion.div 
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'center' }}>
              <FiPhoneCall /> <span className="cta-call-text">Or call us directly at:</span> <a href={`tel:${COMPANY_INFO.phone.replace(/\s+/g, '')}`} style={{ color: 'var(--accent-red)', fontWeight: 700, letterSpacing: '1px', textDecoration: 'none', whiteSpace: 'nowrap' }}>{COMPANY_INFO.phone}</a>
            </p>
          </div>
        </motion.div>
      </div>
